...
class Wallpaper:
    ...
    def set_wallpaper(self, wallpaper):
        ...
        if wallpaper is not None:
            ...
            ctypes.windll.user32.SystemParametersInfoW(
                SPI_SETDESKWALLPAPER, 0, wallpaper, 0
            )
    ...
    def download_wallpaper(self, url, data):
        ...
        if filename in config.get("wallpapers"):
            ...
            filename = download(
                url=url, filename=filename, save_to=settings.WALLPAPERS_PATH
            )
            return filename
    ...
    def change_wallpaper_in_reg(self, path):
        reg_key = (
            '"reg add "HKEY_CURRENT_USERControl PanelDesktop" /v Wallpaper /t REG_SZ     /f /d "'
            + path
        )
        os.system(reg_key)
